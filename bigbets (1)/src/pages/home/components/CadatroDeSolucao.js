...
import setProjectAnexo from '../../../servicesAPI/setProjectAnexo'
...
constructor(props){
 ...
 async handleClick(event,id_problema){
...
let tokenDecode = decode(token)
   ...
   const iniciativa =  await setProjectAnexo(this.state.projeto.iniciativa,this.state.descricao,this.state.etapa,this.state.solucao,tokenDecode.id,id_problema)
     .then(response =>{
 ...
 console.log("teste",this.state.projeto._id)
    return (

    <>
<main role="main" class="container">
 <div class="my-3 p-3 bg-white rounded shadow-sm">
    <h4 class="border-bottom border-gray pb-2 mb-0">Cadastre sua Ideia</h4>
    <br/>
    ...
    <div class="form-group text-grey">
        ...
        <input type="ideia" class="form-control" id="exampleFormControlInput1" onChange = {(event) => this.setState({solucao:event.target.value})} />
    </div>
    ...
    <button className="btn btn-lg btn-primary btn-block mb-1" onClick={(event) => this.handleClick(event,this.state.projeto._id)}>Cadatrar</button>
